...
def eval_seq(opt, dataloader, data_type, result_filename, save_dir=None, show_image=True, frame_rate=30):
    ...
    for i, (path, img, img0) in enumerate(dataloader):
        ...
        online_targets = tracker.update(blob, img0)   #blob加工后的图  img0原图
        online_tlwhs = []
        online_ids = []
        #online_scores = []
...
def main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo',
    ...
    if opt.AFLink:
        ...
        dataset = LinkData('', '')
        # dataset = LinkData('/home/duer/Duer/project/fairmot/FairMOT-master/data/MOT17/images/train', 'val')

    for seq in seqs:
        output_dir = os.path.join(data_root, '..', 'outputs', exp_name, seq) if save_images or save_videos else None
        logger.info('start seq: {}'.format(seq))
        dataloader = datasets.LoadImages(osp.join(data_root, seq, 'img1'), opt.img_size) #1088,608  位置 大小
        ...
        if opt.AFLink:
            ...
            linker = AFLink(
                ...
                model=model,
                dataset=dataset,
                thrT=(0, 30),  # 0(-10, 30) for CenterTrack, FairMOT, TransTrack. -10,0.05/0,0.10
                thrS=70,#75      68,69,70,71,72,73=76.1  0/70/0.05 -10/70/0.10 195
                thrP=0.05  #0.05 0.10 for CenterTrack, FairMOT, TransTrack.    05 10
            )
            linker.link()
...
if __name__ == '__main__':
    ...
    main(opt,
         data_root=data_root,
         seqs=seqs,
         exp_name='mot17_half_train',  #mot20更改
         show_image=False,
         save_images=False,
         save_videos=False)
